...
## Input files
The codes reads in 2 input files, a power spectrum (e.g. powerspectrum.csv, which is a file with 1 column and 2 rows, a list of k values and a list of P(k) values). The second file is an should have the following measurements for galaxies: right ascension, declination, observed spectroscopic redshift, observed log-distance ratio, observed log-distance ratio uncertainty/error, n(r) (mean number density of objects (per Mpc * 1e6) in the survey at the point the galaxy lies at). 
...
$\hat{x} = \cos(\mathrm{RA})\cos(\mathrm{Dec})$,
$\hat{y} = \sin(\mathrm{RA})\cos(\mathrm{Dec})$, 
$\hat{z} = \sin(\mathrm{Dec})$. 
